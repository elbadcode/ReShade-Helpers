...
float4 BorderPass(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
...
const float EdgeOffset = Pause ? 1 : pingpong.y > 0 ?  0 + speed : speed ; 
	...
	float2 border = (BUFFER_PIXEL_SIZE * border_width_variable) *EdgeOffset; // Translate integer pixel width to floating point
