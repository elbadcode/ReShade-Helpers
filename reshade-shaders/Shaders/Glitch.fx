...
uniform int random_value < source = "random"; min = -0.2; max = 0.1232; >;

uniform int random_value2 < source = "random"; min = 0; max = 10; >;
...
{
    bool Pause = pingpongfast.x * pingpongslow.x > 0;
    
       float speed = pingpong.y > 0 ? pingpong.x : 1 - pingpong.x;

    speed *=  (1 - Pause);
    
const float EdgeOffset = Pause ? frametime * 10 : random_value * pingpong.y ;
float Amount = (Timer ) % EdgeOffset == 0 ? random_value2 : abs(framecount) % 132 == 0 ? EdgeOffset *1 : 0;
...
float4 PS_Glitch ( float4 pos : SV_Position, float2 fragCoord : TEXCOORD) : SV_Target
{
	...
	if (bUseUV) {
		Amount = 0; // Just erase this line if you want to use the control at the top
	}
